...
def create_url(url: schemas.URLBase, title: str, desc: str, x:float, y:float, floor:int, db: Session = Depends(get_db)):
    if not validators.url(url.target_url):
        raise_bad_request(message="Your provided URL is not valid")

    db_url = crud.create_db_url(db=db, url=url, title=title, desc=desc, x=x, y=y, floor=floor)
